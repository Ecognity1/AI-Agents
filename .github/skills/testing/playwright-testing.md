...
Generated browser tests must be executed in headed mode when the environment supports GUI browser execution.
...
Do not claim headed execution succeeded when it was not performed.
...
- ALWAYS run Playwright in headed mode when required and supported.
